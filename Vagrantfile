...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    echo "Downloading patched version of erlang"
    wget http://s3.amazonaws.com/downloads.basho.com/erlang/otp_src_R16B02-basho8.tar.gz

    echo "Installing patched version of erlang"
    tar zxvf otp_src_R16B02-basho8.tar.gz
    cd OTP_R16B02_basho8
    ./otp_build autoconf
    ./configure && make && sudo make install

    echo "Confirm Erlang installed successfully"
    echo $(which erl)
    cd

    echo "Install missing security lib"
    apt-get install -y libpam0g-dev

    echo "Downloading riak source"
    git clone git://github.com/basho/riak.git

    echo "Installing riak source"
    cd riak
    make locked-deps
    make devrel
